...
```
 ...
 ─────────    ──────────    ────────────    ────────────    ──────────    ──────────
 디자인        콘텐츠        스타일          다이어그램      HTML/PPTX     검증
 소스 선택     수집          프리뷰          (선택)         생성          & 전달

 프리셋 or     주제,         3가지 실시간    Excalidraw     단일 HTML     스크린샷
 커스텀 PPTX   목적,         프리뷰 →        흐름도,        + CSS 변수    검증,
 임포트?       슬라이드 수   시각적 선택     아키텍처       + 애니메이션  PPTX 내보내기
```
